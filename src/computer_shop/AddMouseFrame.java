 ...
 *         <code>Mouse</code> to stock. Displays fields that <code>Admin</code>
 *         needs to complete in order to add new <code>Mouse</code>:
 *         <ul>
 *         <li>Barcode</li>
 *         <li>Brand</li>
 *         <li>Colour</li>
...
public class AddMouseFrame extends JFrame {
	...
	public static void main(ArrayList<Barcode> BarcodeArray) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddMouseFrame frame = new AddMouseFrame(BarcodeArray);
	 ...
	 * Create the frame.
	 */
	public AddMouseFrame(ArrayList<Barcode> BarcodeArray) {
		...
		lblButtons.setForeground(new Color(255, 204, 0));
		lblButtons.setFont(new Font("Courier New", Font.BOLD, 16));
		lblButtons.setBackground(SystemColor.controlDkShadow);
		lblButtons.setBounds(61, 470, 125, 35);
		...
		jsButtons = new JSpinner();
		jsButtons.setModel(new SpinnerNumberModel(new Integer(0), new Integer(0), null, new Integer(1)));
		jsButtons.setBounds(271, 466, 98, 40);
		contentPane.add(jsButtons);
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					MouseType Type = MouseType.valueOf(cbType.getSelectedItem().toString());
					int Buttons = Integer.parseInt(jsButtons.getValue().toString());
					if (Barcode.checkIfCorrect(Barcode.getValue())) {
						...
						if (!(checkIfBarcodeInArray(BarcodeArray, Barcode))) {// if it is not in stock it is added to
																				// stock
							Mouse m1 = new Mouse(Barcode, Type, Brand, Colour, connectivity, Quantity, OriginalCost,
									RetailPrice, Buttons);
							System.out.println(m1.toFileLine());
							WritingToFile wtf = new WritingToFile("Stock.txt", m1.toFileLine());
							wtf.addToFile();
		...
		JLabel lblInfo = new JLabel("Add Mouse details");
		...
		lblInfo.setBounds(133, 16, 274, 33);
		...
		cbType.setModel(new DefaultComboBoxModel(MouseType.values()));
